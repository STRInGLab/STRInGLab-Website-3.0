(function(){const path=location.pathname.split('/').pop()||'index.html';const navHTML=`
  ...
  </footer>`; const navSlot=document.getElementById('nav-slot');const footSlot=document.getElementById('footer-slot');if(navSlot)navSlot.outerHTML=navHTML;if(footSlot)footSlot.outerHTML=footHTML; const route=(document.body.dataset.route||'').toLowerCase();document.querySelectorAll('.nav-link').forEach(l=>{if(l.dataset.r===route)l.classList.add('active');}); const navToggle=document.querySelector('.nav-toggle');const navLinks=document.getElementById('primaryNav');if(navToggle&&navLinks){navToggle.addEventListener('click',()=>{const isOpen=document.body.classList.toggle('nav-open');navToggle.setAttribute('aria-expanded',String(isOpen));navToggle.setAttribute('aria-label',isOpen?'Close navigation':'Open navigation');});navLinks.querySelectorAll('a').forEach(link=>{link.addEventListener('click',()=>{document.body.classList.remove('nav-open');navToggle.setAttribute('aria-expanded','false');navToggle.setAttribute('aria-label','Open navigation');});});} const nav=document.getElementById('topnav');function onScroll(){const y=window.scrollY;nav.classList.toggle('scrolled',y>20);let onPaper=false;document.querySelectorAll('.paper-bg').forEach(s=>{const r=s.getBoundingClientRect();if(r.top<=60&&r.bottom>=60)onPaper=true;});nav.classList.toggle('on-paper',onPaper);}window.addEventListener('scroll',onScroll,{passive:true});onScroll(); const io=new IntersectionObserver((entries)=>{entries.forEach(e=>{if(e.isIntersecting){e.target.classList.add('in');io.unobserve(e.target);}});},{threshold:0.12});document.querySelectorAll('.card, .sec-head > *').forEach(el=>io.observe(el));})();
