...
function fetchData($pdo) {
    ...
    if ($result) {
        foreach ($result as $row) {
            $formattedDate = date('Y-m-d', strtotime($row['payout_date']));
            $attachmentLink = $row['attachment'] ? "<a href='{$row['attachment']}' target='_blank' class='btn btn-outline btn-outline-dashed btn-outline-success btn-active-light-success'>View↗︎</a>" : "No Attachment";

            // Split the expense description into words
            $expenseDescriptionWords = explode(' ', $row['expense_description']);
            $shortDescription = implode(' ', array_slice($expenseDescriptionWords, 0, 2)) . '...';

			echo "<tr>  
				<td>{$row['resource_name']}</td>  
				<td>{$row['identification']}</td>  
				<td title='{$row['expense_description']}'>{$shortDescription}</td>  
				<td>{$row['project_name']}</td>  
				<td>{$row['payout_amount']}</td>  
				<td>{$row['payout_source']}</td>  
				<td>{$formattedDate}</td>  
				...
				<td class='text-end'>  
					...
					<button class='btn btn-bg-light btn-color-muted btn-active-color-primary btn-sm px-4 badge-light-warning edit-btn' data-id='{$row['id']}'>Edit</button>  
				</td>  
		  </tr>";  
        }
    } else {
...
function getResourceNames($pdo) {
    $sql = "SELECT Name FROM resourceDetails";
    $stmt = $pdo->query($sql);
    return $stmt->fetchAll(PDO::FETCH_ASSOC);
}
$resourceNames = getResourceNames($pdo);

function getProjectNames($pdo) {
    $sql = "SELECT Name FROM projectDetails";
    $stmt = $pdo->query($sql);
    return $stmt->fetchAll(PDO::FETCH_ASSOC);
}
$projectNames = getProjectNames($pdo);

function getIdentification($pdo) {
    $sql = "SELECT identification FROM resourceDetails";
    $stmt = $pdo->query($sql);
    return $stmt->fetchAll(PDO::FETCH_ASSOC);
}
$identifications = getIdentification($pdo);
?>
<!DOCTYPE html>
<!--
Author: Keenthemes
...
<html lang="en">
	...
	<head>
<base href="./" />
		<title>S.T.R.In.G - Expense</title>
		<meta charset="utf-8" />
	...
	<body id="kt_app_body" data-kt-app-layout="dark-sidebar" data-kt-app-header-fixed="true" data-kt-app-sidebar-enabled="true" data-kt-app-sidebar-fixed="true" data-kt-app-sidebar-hoverable="true" data-kt-app-sidebar-push-header="true" data-kt-app-sidebar-push-toolbar="true" data-kt-app-sidebar-push-footer="true" data-kt-app-toolbar-enabled="true" class="app-default">
							...
							<div id="kt_app_toolbar" class="app-toolbar py-3 py-lg-6">
								...
								<div id="kt_app_toolbar_container" class="app-container container-xxl d-flex flex-stack">
									...
									<div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
										<!--begin::Title-->
										<h1 class="page-heading d-flex text-gray-900 fw-bold fs-3 flex-column justify-content-center my-0">Expense Details Dashboard</h1>
									</div>
									<!--end::Page title-->
									<!--begin::Actions-->
									<div class="d-flex align-items-center gap-2 gap-lg-3">
										<!--begin::Primary button-->
										<a href="#" class="btn btn-sm fw-bold btn-primary" data-bs-toggle="modal" data-bs-target="#kt_modal_new_target">Add Expense</a>
										<!--end::Primary button-->
									</div>
									<!--end::Actions-->
								</div>
										...
										<div class="d-flex flex-wrap flex-sm-nowrap">
											...
											<div class="me-7 mb-4">
												<div class="symbol symbol-100px symbol-lg-160px symbol-fixed position-relative">
													<img src="assets/media/avatars/expenses.jpeg" alt="image" />
                                            ...
                                            <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_ecommerce_report_sales_table">
												<thead>
													<tr class="text-start text-gray-500 fw-bold fs-7 text-uppercase gs-0">
														<th class="min-w-125px dt-orderable-asc dt-orderable-desc">Resource Name</th>
														<th class="min-w-125px dt-orderable-asc dt-orderable-desc">Identification</th>
														<th class="min-w-125px dt-orderable-asc dt-orderable-desc">Expense Description</th>
														<th class="min-w-125px dt-orderable-asc dt-orderable-desc">Project Name</th>
														<th class="min-w-125px dt-orderable-asc dt-orderable-desc">Payout Amount</th>
														<th class="min-w-125px dt-orderable-asc dt-orderable-desc">Payout Source</th>
														<th class="min-w-125px dt-orderable-asc dt-orderable-desc">Payout Date</th>
														<th class="min-w-125px dt-orderable-asc dt-orderable-desc">Attachment</th>
		...
		<div class="modal fade" id="kt_modal_new_target" tabindex="-1" aria-hidden="true">
			...
			<div class="modal-dialog modal-dialog-centered mw-650px">
				...
				<div class="modal-content rounded">
                    ...
                    <form id="client_details_form" class="form" method="post" enctype="multipart/form-data" action="submit_expense.php">
						<div class="mb-13 text-center">
							<h1 class="mb-3">Expense Details</h1>
						</div>
						<div class="row g-9 mb-8">
							<div class="col-md-6 fv-row">
								<label class="required fs-6 fw-semibold mb-2">Resource Name</label>
								<select class="form-select form-select-solid" data-control="select2" data-hide-search="true" data-placeholder="Select..." name="ResourceName" id="resource_select">
									<option></option>
									<?php foreach ($resourceNames as $resource): ?>
										<option value="<?php echo htmlspecialchars($resource['Name']); ?>"><?php echo htmlspecialchars($resource['Name']); ?></option>
									<?php endforeach; ?>
								</select>
							</div>
							<div class="col-md-6 fv-row">
								<label class="required fs-6 fw-semibold mb-2">Identification</label>
								<select class="form-select form-select-solid" data-control="select2" data-hide-search="true" data-placeholder="Select..." name="identification" id="identification">
									<option></option>
									<option value="internal">Internal</option>
									<option value="reimbursement">Reimbursement</option>
									<option value="invoice">Invoice</option>
									<?php foreach ($identifications as $identification): ?>
										<option value="<?php echo htmlspecialchars($identification['identification']); ?>"><?php echo htmlspecialchars($identification['identification']); ?></option>
									<?php endforeach; ?>
								</select>
							</div>
						</div>
						<div class="row g-9 mb-8">
							<div class="col-md-12 fv-row">
								<label class="required fs-6 fw-semibold mb-2">Expense Description</label>
								<textarea type="text" class="form-control form-control-solid" placeholder="Enter Expense Description" name="expenseDescription"></textarea>
							</div>
						</div>
						<div class="row g-9 mb-8">
							<div class="col-md-6 fv-row">
								...
								<select class="form-select form-select-solid" data-control="select2" data-hide-search="true" data-placeholder="Select..." name="ProjectName" id="project_select">
									<option></option>
									<option value="internal_expense">Internal Expense</option>
							...
							<div class="col-md-6 fv-row">
								<label class="required fs-6 fw-semibold mb-2">Payout Amount</label>
								<input type="number" step="0.01" class="form-control form-control-solid" placeholder="Enter Amount" name="payoutAmount" required />
							</div>
							<div class="col-md-6 fv-row">
								<label class="required fs-6 fw-semibold mb-2">Payout Source</label>
								<input type="text" class="form-control form-control-solid" placeholder="Enter Source" name="payoutSource" required />
							</div>
							<div class="col-md-6 fv-row">
								<label class="required fs-6 fw-semibold mb-2">Payout Date</label>
								<input type="date" class="form-control form-control-solid" placeholder="Payout Date" name="PayoutDate" required />
							</div>
						</div>
						<div class="row g-9 mb-8">
							...
							<div class="col-md-12 fv-row">
								...
								<textarea type="text" class="form-control form-control-solid" placeholder="Enter Payment Description" name="notes"></textarea>
							</div>
						</div>
		...
		<script>
            ...
            var myDropzone = new Dropzone(id, { // Make the whole body a dropzone
                url: "submit_expense.php", // Set the url for your upload script location
                parallelUploads: 20,
		...
		<script>  
			$(document).ready(function(){  
					...
					$.ajax({  
						...
						success: function(data) {  
							if (data.error) {  
								...
								$('select[name="ResourceName"]').val(data.Resource_Name).trigger('change');  
								$('select[name="identification"]').val(data.Identification).trigger('change');  
								$('textarea[name="expenseDescription"]').val(data.Expense_Description);  
								$('select[name="ProjectName"]').val(data.Project_Name).trigger('change');  
								$('input[name="payoutAmount"]').val(data.Payout_Amount);  
								$('input[name="payoutSource"]').val(data.Payout_Source);  
								$('input[name="PayoutDate"]').val(data.Payout_Date);  
				...
				$('#kt_modal_new_target').on('hidden.bs.modal', function () {  
					...
					$('input[name="id"]').val('');  
					$('select[name="ResourceName"]').val('').trigger('change');  
					$('select[name="identification"]').val('').trigger('change');  
					$('select[name="ProjectName"]').val('').trigger('change');  
