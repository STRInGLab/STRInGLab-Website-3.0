...
function calculateProgress($actualStartDate, $endDate) {
    if (empty($actualStartDate) || empty($endDate)) {
        return 0;
    }
    $start = strtotime($actualStartDate);
    ...
    return min(100, max(0, ($now - $start) / ($end - $start) * 100));
}

function determineStatus($actualEndDate) {
    if (empty($actualEndDate)) {
        return 'primary'; // In Progress
    } elseif (strtotime($actualEndDate) <= time()) {
...
function displayProjectDetails($pdo) {
    ...
    echo '<script>console.log("Number of rows: ' . count($result) . '");</script>';
    echo '<script>console.log(' . json_encode($result) . ');</script>';

    if (count($result) > 0) {
        // Output data of each row
        foreach ($result as $row) {
            // Calculate progress based on Actual_Start_Date and End_Date
            $progress = calculateProgress($row['Actual_Start_Date'], $row['End_Date']);
    
            // Determine the status based on Actual_End_Date
            $status = determineStatus($row['Actual_End_Date']);
    
            // Generate HTML
            echo '<div class="col-md-6 col-xl-4">';
            echo '<a href="apps/projects/project.html" class="card border-hover-primary">';
            echo '<div class="card-header border-0 pt-9">';
            echo '<div class="card-title m-0">';
            echo '<div class="symbol symbol-50px w-50px bg-light">';
            echo '<img src="assets/media/svg/brand-logos/plurk.svg" alt="image" class="p-3" />';
            echo '</div>';
            echo '</div>';
            echo '<div class="card-toolbar">';
            echo '<span class="badge badge-light-' . $status . ' fw-bold me-auto px-4 py-3">' . ucfirst($status) . '</span>';
            echo '</div>';
            echo '</div>';
            ...
            echo '<div class="symbol symbol-35px symbol-circle" data-bs-toggle="tooltip" title="Emma Smith">';
            echo '<img alt="Pic" src="assets/media/avatars/300-6.jpg" />';
            echo '</div>';
            echo '<div class="symbol symbol-35px symbol-circle" data-bs-toggle="tooltip" title="Rudy Stone">';
            echo '<img alt="Pic" src="assets/media/avatars/300-1.jpg" />';
            echo '</div>';
            echo '<div class="symbol symbol-35px symbol-circle" data-bs-toggle="tooltip" title="Susan Redwood">';
            echo '<span class="symbol-label bg-primary text-inverse-primary fw-bold">S</span>';
            echo '</div>';
            echo '</div>';
            echo '</div>';
            echo '</a>';
